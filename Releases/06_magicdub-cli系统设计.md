...
费用：本批 attempt=1 → `cost_of_translation`；≥2 → `cost_of_duration_fitting.translation`。
...
- 费用：attempt=1 → `cost_of_tts`；≥2 → `cost_of_duration_fitting.tts`
...
- `cost`：sep／asr／translation／tts／duration_fitting.{translation,tts}／**total**（ledger 汇总）
