...
> **状态（2026-09-13）：历史参考，当前不执行。** MagicDub 已取消 `magicdub-cloud`，原 `magicdub` 暂时保留；当前只推进主要自用的 [magicdub-skills](05_magicdub-skill项目规划.md)。本文旧 Web 目标、架构及部署步骤不再作为当前要求；未来确有对外 Web 需求时重新评估。
...
3. `separate_audio(source_audio) -> speech_audio, residual_audio`
  - **做什么**：把音轨分离为干声与残余/环境轨（对应 sam-audio 的 residual）。
  - **落盘**：
    - `stems/{job_id}/speech.wav`
    - `stems/{job_id}/residual.wav`
...
9. `refine_target_audio(sentences) -> sentences`
  ...
  - **规则（来自草稿，建议作为默认）**
    - 若 0.8 \le \frac{ttsduration}{targetduration} \le 1.2：用 FFmpeg 轻微拉伸/压缩到等长
...
10. `assemble_target_video(source_video, residual_audio, aligned_sentence_audios, timeline) -> target_video`
  - **做什么**：把 source_video 静音后，混入 `residual_audio`（保持连续），再把每句 `aligned_sentence_audio` 按 `sentence_start_time_ms` 放置。
  - **执行位置**：阿里云 FC（FFmpeg）。
...
依据 [取消 Cloud 并收敛为自用技能集](../.records/events/2026-09/2026-09-13_013354_取消Cloud并收敛为自用magicdub-skills.md)，本文退出当前实施范围；旧技术正文保留供历史追溯。
